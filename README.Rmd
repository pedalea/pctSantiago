...
```{r, eval=FALSE}
d_orig = readr::read_csv2("~/data/data/raw/od/EOD2012_Stgo.csv")
n = names(d_orig) 
...
d_agg_zone = d %>% 
  ...
  summarise(n = n(), av_weight = mean(`Total (FV*FP)`), all = n() * mean(`Total (FV*FP)`))
...
pcycle_current = runif(n = nrow(od_agg_zone_sub), min = 0.01, max = 0.06)
pcycle_govtarget = pcycle_current * 2
pcycle_godutch = pcycle_current * 8
od_agg_zone_sub$bicycle = od_agg_zone_sub$all * pcycle_current
